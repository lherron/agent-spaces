...
/** The launchd stdout/stderr file is copy-truncated at start above this size. */
export const LAUNCHD_LOG_MAX_BYTES = 16 * 1024 * 1024
...
export function renderLaunchdScript(input: {
  ...
  env: Record<string, string>
  logPath: string
  /** aspd's own bounded request log (`aspd serve --log`). */
  ...
  const format = `${JSON.stringify({
    ...
    requestLogPath: input.requestLogPath,
  })
    .replaceAll('%', '%%')
    .replace('"pid":0', '"pid":%s')
  ...
  return [
    ...
    // Bound the supervisor's file: aspd writes requests to its own rotated
    // log, so this file holds lifecycle and fatal lines only. No aspd is
    // writing it here; reopening stdout/stderr in append mode afterwards makes
    // the truncation safe however the supervisor opened the file.
    `if [ -f ${shellQuote(input.logPath)} ] && [ "$(wc -c <${shellQuote(input.logPath)})" -gt ${LAUNCHD_LOG_MAX_BYTES} ]; then cp ${shellQuote(input.logPath)} ${shellQuote(`${input.logPath}.1`)} && : >${shellQuote(input.logPath)}; fi`,
    `exec >>${shellQuote(input.logPath)} 2>&1`,
    ...
    `exec /usr/bin/env -i ${env} ${shellQuote(join(input.releasePath, 'aspd'))} serve --socket ${shellQuote(input.socketPath)} --log ${shellQuote(input.requestLogPath)}`,
    '',
  ].join('\n')
}
...
function writeLaunchScript(
  ...
  const script = renderLaunchdScript({
    ...
    logPath,
    requestLogPath: join(p.logs, 'aspd.log'),
  })
