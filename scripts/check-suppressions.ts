...
const baselineMeta = {
  ...
  warning:
    'future suppressions use EXCEPTION(); baseline update is for intentional grandfather/reset only and must be reviewed',
}
...
function suppressionMatches(text: string): { kind: string; rest: string; token: string }[] {
  const matches: { kind: string; rest: string; token: string }[] = []
  const cleaned = cleanCommentText(text)
  const patterns = [
  ...
  for (const pattern of patterns) {
    ...
    matches.push({ kind, rest, token: match[0] })
  }
...
function diagnosticFor(suppression: Suppression): string {
  return [
    ...
    `  ${suppression.text}`,
    '  fix: add `EXCEPTION(<ticket>): <reason>` above it, or remove the suppression.',
    '  Do not suppress check-suppressions; update .suppression-baseline.json only for reviewed grandfather/reset changes.',
  ].join('\n')
}
