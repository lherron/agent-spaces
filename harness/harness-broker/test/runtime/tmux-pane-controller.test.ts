...
describe('TmuxPaneController', () => {
  ...
  test('matches a pasted command that capture-pane hard-wraps across lines', async () => {
    ...
    let submitted = false
    const cmd =
      'exec bun /Users/x/praesidium/agent-spaces/harness/harness-broker/src/runtime/tmux-launch-runner.ts --launch-file /tmp/harness-broker/codex-hooks.sock.codex.launch.json'
