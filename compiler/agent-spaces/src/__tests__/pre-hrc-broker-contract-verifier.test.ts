...
describe('runPreHrcBrokerContractHarness contract gate', () => {
  ...
  test('interactive-tmux drives turns 1 and 2 through broker input without a launch-prompt turn', async () => {
    ...
    expect(result.interactiveTmux.driverTmuxLoadedText).toEqual(
      expect.arrayContaining([firstInput, secondInput])
    )
    ...
    const launchCommand = result.interactiveTmux.driverTmuxLoadedText.find((text) =>
