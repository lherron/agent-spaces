 ...
 *     stdout contains "apps/cli" (stable self-command hit).
 *
...
// Test file lives at compiler/agent-spaces/src/__tests__/ → 4 levels up.
...
describe('discovery tools (T-04397)', () => {
  test('find-entry-points: exits 0, emits file:line\\trole hits, contains apps/cli', async () => {
    // topic "self" → the asp self command group under apps/cli is a stable hit
    ...
    // Stable known hit: asp self command group lives under apps/cli
    expect(stdout).toContain('apps/cli')
  })

  test('explain-area: exits 0, emits all section markers, imported-by section is non-empty', async () => {
    const { exitCode, stdout } = await runScript('explain-area.ts', ['contracts/agent-scope'])
    ...
    expect(stdout).toMatch(/(?:apps|compiler|contracts|core|drivers|harness)\/.+:\d+/)
    ...
    expect(afterSection).toMatch(/(?:apps|compiler|contracts|core|drivers|harness)\/.+:\d+/)
  }, 30_000)
})
