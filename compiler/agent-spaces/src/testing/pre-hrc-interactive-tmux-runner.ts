 ...
 *
 * Boundary note: this file lives under compiler/agent-spaces/src/testing/**,
...
function postHookViaBridge(options: {
  ...
  const harnessBrokerJs = join(options.repoRoot, 'harness/harness-broker/bin/harness-broker.js')
  return new Promise((resolvePromise, reject) => {
    const proc = spawn(
      'bun',
...
export async function runInteractiveClaudeTmuxSession(
  ...
  const hookBridgeCommand = `bun ${join(options.repoRoot, 'harness/harness-broker/bin/harness-broker.js')} claude-hook`
