...
async function runInteractiveTmuxInvocation(
  ...
  try {
    ...
    const { createInvocationManager } = await importSibling(
      '../../../../harness/harness-broker/src/invocation-manager'
    )
    const { createInvocationEventSequencer } = await importSibling(
      '../../../../harness/harness-broker/src/events'
    )
    const { parseDispatchEnv } = await importSibling(
      '../../../../harness/harness-broker/src/runtime/env'
    )
    const driverSpecifier = [
      '../../../../harness/harness-broker/src',
      'drivers',
      'claude-code-tmux',
      'driver',
    ].join('/')
