...
`AGENT_GENERATION`, `AGENT_PROJECT_ROOT`, and `AGENT_ACTOR`. `AGENT_SCOPE_REF` is durable
...
`ASP_PROJECT`, `AGENTCHAT_ID`, `HRC_SESSION_REF`, `HRC_RUN_ID`,
`HRC_HOST_SESSION_ID`, and `HRC_GENERATION` (the migration alias for
`AGENT_GENERATION`).
