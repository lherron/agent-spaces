...
async function rewritePublishedPackageJson() {
  // Dev needs workspace names in `dependencies` so Bun workspace linking
  // and test resolution work, but published consumers installing under Bun
  // hit the npm registry for `*` specs and 404 on private names. Move
  // workspace names into `optionalDependencies` for the published tarball
  // so consumers skip them when registry lookup fails — the bundled
  // node_modules payload has the real code. Postpack reverts via git.
  const workspaceNames = new Set(WORKSPACES.map((w) => w.dest))
  ...
  if (pkg.dependencies) {
    for (const name of Object.keys(pkg.dependencies)) {
      if (workspaceNames.has(name)) {
        moved[name] = pkg.dependencies[name]
        delete pkg.dependencies[name]
      }
    }
  }
  if (Object.keys(moved).length > 0) {
    pkg.optionalDependencies = { ...(pkg.optionalDependencies ?? {}), ...moved }
  }
  // Drop the `bun` export condition — it points at TS source that isn't
  // shipped and would shadow the `import` condition for Bun consumers.
  if (pkg.exports) {
...
async function main() {
  ...
  await rewritePublishedPackageJson()
}
