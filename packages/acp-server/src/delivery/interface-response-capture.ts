...
): InterfaceResponseCaptureHandler {
  ...
  return async (event: UnifiedSessionEvent): Promise<void> => {
    try {
      ...
      deps.interfaceStore.deliveries.enqueue({
        ...
        ...(context.source.threadRef !== undefined ? { threadRef: context.source.threadRef } : {}),
        ...(!replyAnchorConsumed
          ? {
      ...
      console.error(`[acp-server] interface response capture skipped for run ${deps.runId}:`, error)
    }
  }
}
