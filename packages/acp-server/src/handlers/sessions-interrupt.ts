...
export const handleInterruptSession: RouteHandler = async ({ params, deps }) => {
  ...
  const result = await hrcClient.interrupt(latest.runtimeId)

  return json({ ok: result.ok, runtimeId: latest.runtimeId })
}
