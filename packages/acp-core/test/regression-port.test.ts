...
describe('task-core regression port', () => {
  test('task CRUD preserves version 0 and preset pinning', () => {
    ...
    store.createTask(createTestTask({ taskId: 'task-001', phase: 'open' }))
  ...
  test('versioned transitions advance monotonically', () => {
    const store = new InMemoryAcpWorkflowStore()
    store.createTask(createTestTask({ taskId: 'task-002', phase: 'open' }))

    const first = store.transition('task-002', {
      toPhase: 'red',
      actor: { agentId: 'tracy', role: 'triager' },
      evidence: [createEvidence('tdd_red_bundle')],
      expectedVersion: 0,
    })
    ...
    if ('task' in second) {
      ...
      expect(second.task.phase).toBe('green')
    }
  })
  ...
  test('preset objects are immutable and task pins survive transitions', () => {
    const store = new InMemoryAcpWorkflowStore()
    store.createTask(createTestTask({ taskId: 'task-005', phase: 'open' }))
    ...
    expect(() => {
      ;(preset.transitionPolicy as Array<{ toPhase: string }>).push({ toPhase: 'oops' })
    }).toThrow()
  ...
  test('transition history is recorded in order and unknown tasks return NOT_FOUND', () => {
    const store = new InMemoryAcpWorkflowStore()
    store.createTask(createTestTask({ taskId: 'task-006', phase: 'open' }))
    ...
    if ('transitions' in transitions) {
      ...
      expect(transitions.transitions[0].from.phase).toBe('open')
      expect(transitions.transitions[0].to.phase).toBe('red')
      expect(transitions.transitions[0].transitionEventId).toBeTruthy()
      expect(transitions.transitions[0].timestamp).toBeTruthy()
      expect(transitions.transitions[1].to.phase).toBe('green')
      expect(transitions.transitions[1].actor.agentId).toBe('larry')
    }
