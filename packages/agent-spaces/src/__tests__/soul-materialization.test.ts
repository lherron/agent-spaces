...
describe('system prompt materialization (T-00900)', () => {
  test(
    'SOUL.md content appears as --system-prompt value in argv',
    async () => {
...
describe('HEARTBEAT.md in heartbeat mode (T-00900)', () => {
  test(
    '--system-prompt contains both SOUL.md and HEARTBEAT.md in heartbeat mode',
    async () => {
      const { agentRoot, aspHome, cleanup } = createTempAgentRoot({
        soulContent: '# Alice\n\nYou are Alice.',
...
describe('no HEARTBEAT.md in query mode (T-00900)', () => {
  test(
    '--system-prompt contains SOUL.md but NOT HEARTBEAT.md in query mode',
    async () => {
      const { agentRoot, aspHome, cleanup } = createTempAgentRoot({
        soulContent: '# Alice\n\nYou are Alice.',
...
describe('empty SOUL.md edge case (T-00900)', () => {
  test(
    'empty SOUL.md still produces --system-prompt flag',
    async () => {
      const { agentRoot, aspHome, cleanup } = createTempAgentRoot({
        soulContent: '',
      })
