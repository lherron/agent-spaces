...
import { computeReachableIntegrities, computeReachableCacheKeys } from "./gc.js";
import type { LockFile, SpaceKey, Sha256Integrity } from "@agent-spaces/core";

function createMockLock(spaces: Record<string, { integrity: string; pluginName: string; version: string }>): LockFile {
	const lock: LockFile = {
		lockfileVersion: 1,
		resolverVersion: 1,
