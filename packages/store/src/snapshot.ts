...
import { mkdir, rm, rename, stat, readFile, writeFile, readdir } from "node:fs/promises";
import { createHash } from "node:crypto";
import { extractTree, extractTreeToTemp } from "@agent-spaces/git";
import { computeIntegrity, type IntegrityOptions } from "@agent-spaces/resolver";
import type { SpaceId, CommitSha, Sha256Integrity } from "@agent-spaces/core";
import { asSha256Integrity, SnapshotError } from "@agent-spaces/core";
import { getSnapshotPath, getTempPath, ensureDir, type PathResolver } from "./paths.js";
...
export async function createSnapshot(
	...
	try {
		...
		await writeFile(
			join(tempDir, ".asp-snapshot.json"),
