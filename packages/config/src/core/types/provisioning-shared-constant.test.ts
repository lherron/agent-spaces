 ...
 * exists to forbid.
 *
 * Dependency direction forces the ownership: packages/config depends on agent-scope
 * (packages/config/package.json:69) and agent-scope is zero-dep, so the single
