...
import { validateInvocationSpec } from "../src/schemas";
  ...
  expect(() => validateInvocationSpec(value)).toThrow(
    expect.objectContaining({
      ...
      issues: expect.arrayContaining([
        expect.objectContaining(expectedIssue),
      ]),
    }),
  );
};
...
describe("validateInvocationSpec", () => {
  test("accepts the Codex app-server example from spec section 6.2", () => {
    expect(validateInvocationSpec(specSection62Example)).toEqual(
