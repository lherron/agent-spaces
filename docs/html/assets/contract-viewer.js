(() => {
  ...
  const highlightCode = (value) => {
    ...
    html = html.replace(/\b([A-Za-z_][A-Za-z0-9_]*\??)(?=\s*:)/g, '<span class="syntax-field">$1</span>');
    html = html.replace(/\b(type|Promise|Record|RuntimeCompileRequest|RuntimeCompileResponse|CompiledRuntimePlan|CompileDiagnostic)\b/g, '<span class="syntax-keyword">$1</span>');
