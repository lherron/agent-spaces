...
> Claude Agent SDK selection path. `claude-agent-sdk` is not a public harness
> ID or alias under the active four-harness contract, and this runbook is not
> current acceptance guidance. See `agent-spaces/harness-architecture` and
