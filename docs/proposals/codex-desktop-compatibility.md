...
Final hook result: PASS using the bundled 0.153.3 in the isolated home. Two bounded marker-only model turns produced SessionStart `source=startup` and `source=resume`, both carrying the same thread ID `01a081b1-5cc5-7d83-a939-2f45750b47d6`, transcript_path and cwd. Resume discovery was observed on the first turn after resume, not on the resume RPC alone. No desktop conversation was created for this isolated hook test. The temporary auth-file symlink was removed. Structured readback: `~/praesidium/var/tmp/codex-desktop-contract/hooks-probe.json`.
