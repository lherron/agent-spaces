...
An immutable ASP release can prepare and execute the release-bound
compiler-backed Claude/Pi broker profiles without resolving ASP-owned code from
...
| Release build/inspection | T-08561 direct worker | `just build-asp-release <A-build>` then `just install-asp-release <A> <ns>/releases` and `just inspect-asp-release <ns>/releases/<A>`; inspect `release.json`, `sha256`, `<release>/harness-broker --release-info`, and `drivers --json`. The worker payload reports A; the narrowed three-driver binding table and asset digest validate, and no Pi SDK worker payload is present or claimed. Separately inspect and activate a retained pre-T-08561 v1 release, verify its manifest/payload is unchanged and its compile response omits `hostedDrivers`, and record that its own compiled semantics remain in force. Do not launch a legacy non-Codex preparation. |
