...
| 2026-08-23 | T-07490 | Context-template exec reminders that render session attention need the canonical principal and project during compilation, before the final harness environment exists | check | `packages/agent-spaces/src/__tests__/agent-env-contract.red.test.ts` |
| 2026-09-20 | T-08666 | A generic adapter over a fenced broker must prove the composed start request through the published wire validator plus `installIdentity`/`ensureInvocation`, not `createBroker`+`start` — the direct path skips the wire refusal and the identity gate | check | `compiler/agent-spaces/src/__tests__/resident-participant-adapter.test.ts`; `harness/harness-broker/test/drivers/arris-resident/resident-participant-broker.integration.test.ts` |
