...
|---|---|---|
| **Idle** (warm, no turn running) | **Starts a turn.** Steer text is the turn's input; response carries the new turn identity. Live in the broker for every driver (`steer()` applies the input as an own turn when the invocation is ready) except `arris-resident`, which sets `steerNeverStartsTurn` and forwards it to the host; the host start arm is T-08534. The driver with no `steer` admission class (codex-desktop) is refused at broker admission today; T-08536 makes the HRC steer door fail open and submit the message as an enqueue, recorded in the response and ledger. | **Starts a turn.** Own turn, own run, receipt, waitable. Live. |
