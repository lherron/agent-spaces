...
const compilerSrc = join(repoRoot, 'compiler', 'agent-spaces', 'src')
const turnRunnerSrc = join(repoRoot, 'apps', 'turn-runner', 'src')
...
describe('spaces-turn-runner package boundary', () => {
  ...
  test('AC2: turn-runner uses only declared agent-spaces package exports', () => {
    ...
    const manifest = JSON.parse(
      readFileSync(join(repoRoot, 'compiler', 'agent-spaces', 'package.json'), 'utf8')
