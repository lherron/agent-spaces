...
/** Harness-neutral agent-local discovery and tool environment preparation. */
...
/** Discover optional agent-local skills, commands and executable tools. */
export async function detectAgentLocalComponents(
  ...
  if (!hasSkills && !hasCommands && !hasTools) return undefined
  return {
    agentRoot,
